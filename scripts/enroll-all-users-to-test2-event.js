...
async function main() {
  try {
    // Find the event named 'test2'
    const event = await prisma.event.findFirst({ where: { name: 'test2' } });
    if (!event) {
      console.error("Event named 'test2' not found.");
      process.exit(1);
    }
    ...
    console.log(`\nEnrolled ${enrolled} users to event 'test2'.`);
  } catch (error) {
